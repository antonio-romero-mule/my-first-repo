	...
	<xsl:template match="/">
		...
		<xsl:if test="$display = 'common' or $display = 'all'">
			...
			<xsl:text>
				...
				<!-- This is here for the proper line breaking -->
		...
		<xsl:if test="$display = 'specific' or $display = 'all'">
			...
			<xsl:apply-templates select="/xsd:schema/xsd:element[ @name!='connector' and @name!='endpoint' and @name!='inbound-endpoint' and @name!='outbound-endpoint' and not(starts-with(@name, 'abstract')) and not(contains(@substitutionGroup,'abstract-transformer')) and not(contains(@substitutionGroup,'abstract-filter'))]" mode="single-element"/>
